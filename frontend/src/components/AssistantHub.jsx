...
export default function AssistantHub({ onError, onDataMutation }) {
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      role: 'assistant',
      content: 'Ask me to create tasks, update preferences, or log donations and feedback.',
      actionItems: [],
      automationEvents: [],
    },
  ]);
  ...
  const submitFeedback = async (event) => {
    ...
    try {
      ...
      setChatHistory((prev) => [...prev, { role: 'assistant', content: 'Feedback saved.' }]);
    } catch (err) {
  ...
  return (
    <div className="assistant-hub">
      ...
      <div className="tab-row" role="tablist" aria-label="Assistant Views">
        <button type="button" className={`tab-chip ${activeTab === 'chat' ? 'active' : ''}`} onClick={() => setActiveTab('chat')}>
          Chat
        </button>
        <button type="button" className={`tab-chip ${activeTab === 'tasks' ? 'active' : ''}`} onClick={() => setActiveTab('tasks')}>
          Tasks
        </button>
        <button type="button" className={`tab-chip ${activeTab === 'prefs' ? 'active' : ''}`} onClick={() => setActiveTab('prefs')}>
          Preferences
        </button>
        <button type="button" className={`tab-chip ${activeTab === 'notes' ? 'active' : ''}`} onClick={() => setActiveTab('notes')}>
          Feedback & Giving
        </button>
      </div>

      {activeTab === 'chat' && (
        <div className="assistant-column assistant-column-wide">
          <div className="assistant-chat">
            ...
            <form className="assistant-form" onSubmit={sendChat}>
              <textarea
                ...
                rows={3}
                placeholder="Try: review subscriptions tomorrow"
              />
      ...
      {activeTab === 'notes' && (
        <div className="assistant-layout assistant-layout-compact">
          <div className="assistant-column">
            <div className="assistant-card">
              <h4>Feedback</h4>
                ...
                {donations.slice(0, 5).map((donation) => (
                  <div className="task-item" key={donation.id}>
                    <div>
                      ...
                      <p className="muted">{donation.amount} {preferences?.currency || 'INR'} • {donation.recurring ? 'recurring' : 'one-time'}</p>
                    </div>
